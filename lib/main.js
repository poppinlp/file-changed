...
module.exports = class {
	...
	check(...args) {
		const
			list = args.length ? args : this.list(),
			res = [];
		...
		for (const item of list) {
			...
			getTS(item) !== this._data[item].ts && res.push(item);
		}
	...
	update(...args) {
		const list = args.length ? args : this.check();
		...
		for (const item of list) {
			...
			this._data[item].ts = getTS(item);
			this._data[item].md5 = getMD5(item);
		}
